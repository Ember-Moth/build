...
const Label = component$<PropsOf<typeof HeadlessSelect.Label>>(({ ...props }) => {
  return (
    <HeadlessSelect.Label {...props} class={cn('mb-1.5 font-semibold', props.class)}>
      <Slot />
...
const Trigger = component$<PropsOf<typeof HeadlessSelect.Trigger>>(({ ...props }) => {
  return (
    <HeadlessSelect.Trigger
      {...props}
      class={cn(
        'border-input ring-offset-background placeholder:text-muted-foreground focus:ring-primary flex h-9 w-full items-center justify-between rounded-md border bg-transparent px-3 py-2 text-sm whitespace-nowrap focus:ring-1 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
        props.class,
      )}
    >
      <Slot />
...
const ItemIndicator = component$<PropsOf<typeof HeadlessSelect.ItemIndicator>>(({ ...props }) => {
  return (
